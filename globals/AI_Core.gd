...
func _ready():
	req_level = get_level_req(max_level_reached)
...
func ai_process():
	...
	#add new worker
	if req_level["max_people"] > get_total_people() and data_ai["max_people"] > get_total_people():
		var req = "worker"
		var buyable = true
		if data_ai["worker"] < req_level["worker"]:
			pass
		else:
			if data_ai["melee_barrack"] == 0:
				buyable = false
				yield(build_wheat_field("melee_barrack"), "completed")
#			if data_ai["ranged_barrack"] == 0 and max_level_reached == 9:
#				buyable = false
	...
	#change state worker
	var division = []
	if req_level["wood"] > data_ai["wood"]:
		division.push_back("wood")
	if req_level["food"] > data_ai["food"]:
		division.push_back("food")
	if division.size() == 0:
		return yield(get_tree(), "idle_frame")
	var max_employee_division = req_level["worker"] / division.size()
	var no = 0
	for body in get_tree().get_nodes_in_group("Worker"):
		if body.state == AI_Core.IDLE :
...
func get_total_people():
	return data_ai["worker"] + data_ai["swordman"]
